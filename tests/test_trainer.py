...
def test_trainer_saves_all_artifacts_and_stops_early(tmp_path):
    ...
    for path in (
        "weights/best.pth", "results/run_config.json",
    ...
    # The upstream trainer intentionally keeps only the validation-best state.
    assert not (tmp_path / "weights" / "final.pth").exists()
